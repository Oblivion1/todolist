        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <span>Edit {{$note->name}}</span>
                    <a class="float-right" href="{{Route('note')}}?u={{$note->uuid}}">Back to {{$note->title}}</a>
                </div>

                <div class="card-body">
                    <form method="POST" action="{{Route('savenote')}}?u={{$note->uuid}}">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="content" rows="4" class="form-control{{ $errors->has('content') ? ' is-invalid' : '' }}"
                                    name="content" required autofocus>{{$note->content}}</textarea>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="duration" type="text" class="form-control{{ $errors->has('duration') ? ' is-invalid' : '' }}"
                                    name="duration" value="{{$note->duration}}" autofocus>
